...
def language(name: str):
    ...
    valids = [
        "name", "category", "displayName", "image", "tagline", "entrypoint"
    ]

    for l in langs:
        if l["name"] == name: lang = l
        else: pass
    
    if not lang:
        return flask.abort(404)
    else:
        try: cmd = " ".join(lang["run"]["command"])
        except KeyError: cmd = None

        try: is_temp = bool(lang["template"])
        except KeyError: is_temp = False

        icon = fetch.format_icon(lang["icon"])
        raw = json.dumps(lang, indent=2)
        
        return flask.render_template(
            "lang.html", lang=lang, cmd=cmd, icon=icon,
            is_temp=is_temp, valids=valids, raw=raw
        )
